...
class PureSREngine:
    def __init__(self, device=DEFAUT_CUDA_DEVICE, model_url="https://github.com/xinntao/Real-ESRGAN/releases/download/v0.1.0/RealESRGAN_x4plus.pth"):
        ...
        self.model = CustomRRDBNet().to(self.device)
    ...
    def _load_weights(self, url):
        ...
        loadnet = torch.load(weight_path, map_location=self.device)
        # 官方權重keys  .防止名字對不上
    ...
    def enhance(self, img_bgr, enhance_detail=True):
        ...
        ## Canvs stays in CPU memory to save GPU VRAM, only tiles are moved to GPU for processing
        output_tensor = torch.zeros((batch, channel, output_height, output_width), dtype=torch.float32)
        ...
        for y in range(0, height, self.tile_size):
            for x in range(0, width, self.tile_size):
                ...
                ## back to cpu, convert to float32 for safe cropping and pasting
                output_tile = output_tile.cpu().float()

                del input_tile
                if self.device.type == 'cuda':
                    torch.cuda.empty_cache()  # release GPU memory 
                    gc.collect() ## Reduce Python memory fragmentations to free up 
                ...
                output_tensor[:, :, y_out_start:y_out_end, x_out_start:x_out_end] = \
                    output_tile[:, :, y_crop_start:y_crop_end, x_crop_start:x_crop_end].cpu() # ㄅ一ㄤˋ回原始canvas上   

        # Tensor -> Numpy BGR
        output_tensor = output_tensor.float().squeeze(0).permute(1, 2, 0).clamp_(0, 1)
        output_numpy = (output_tensor.cpu().numpy() * 255.0).astype(np.uint8)
...
if __name__ == '__main__':
    ...
    engine = PureSREngine(device=DEFAUT_CUDA_DEVICE)
